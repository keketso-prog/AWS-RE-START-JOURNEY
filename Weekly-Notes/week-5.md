...
Amazon DynamoDB (The "Powerful" NoSQL Database) 
...
- Blazing Fast: Can handle millions of requests per second with very little delay
 - Fully Managed: AWS handles all the technical maintenance.
 -  You don't have to worry about servers. Scales Automatically: Grows to handle almost any amount of traffic or data without you needing to do anything.
 -  How it Works (Simplified): You store data in Tables (like a filing cabinet), which contain Items (like folders), which are made of Attributes (the individual pieces of data). Everything is identified by a Primary Key.
